...
for layer_number in range(3,4):
    ...
    for epoch in range(epcoh_number):
        for i, (images, labels) in enumerate(train_loader):
            ...
            if binary_g==1:
                # Binarize gradients
                threshold = 0.0  # Set your desired threshold for binarization
                #for param in new_model.parameters():
                for name, param in model.named_parameters():
                    if param.grad is not None:
                        changed_gradient = torch.tensor(1.0)#param.grad.data.abs().mean()
                        param.grad.data = torch.where(param.grad.data > threshold, changed_gradient,-1*changed_gradient)#torch.tensor(1.0), torch.tensor(-1.0))#*torch.max(param.grad.data)
