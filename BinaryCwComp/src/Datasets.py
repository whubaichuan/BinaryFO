...
class X_Image(torch.utils.data.Dataset):
    def __init__(self, data_samples, size=(84, 84), number_samples=60000, dataset='mini',  increment_dif=False, FF_rep=False):
    ...
    def __getitem__(self, index):
        ...
        print(self.normal_imgs[index].shape)
        #print(self.normal_imgs[index].dtype)
        #print(self.normal_imgs[index].type)
        # Load Positive and Negative Samples
        x_pos = transform(self.normal_imgs[index])
        y_pos = self.y_pos[index]
...
class X_Tiny(torch.utils.data.Dataset):
    def __init__(self, data_samples, size=(84, 84), number_samples=60000, dataset='mini',  increment_dif=False, FF_rep=False):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == 'MNIST':
            ...
            #         Resize(84),             # resize shortest side to 224 pixels
            #         CenterCrop(84),     # crop longest side to 224 pixels at center
            #         Normalize((122.4786, 114.2755, 101.3963), (70.4924, 68.5679, 71.8127)),
            #         ToTensor()
            # ])

        #print(self.normal_imgs[index].shape)
        #print(self.normal_imgs[index].dtype)
        #print(self.normal_imgs[index])
        #print(type(self.normal_imgs[index]))
        # Load Positive and Negative Samples
        x_pos = transform(self.normal_imgs[index].transpose(1,2,0))
        y_pos = self.y_pos[index]
