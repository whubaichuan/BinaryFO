...
def configure(args):
    ...
    if args.dataset[:5] == 'CIFAR':
        ...
        if args.dataset == 'MNIST':
            ...
            dataset = ImageFolder(root='./dataset/miniimagenet')
            n_data = len(dataset)
            n_train = int(0.85 * n_data)
            n_valid = 0#int(0.15 * n_data)
            n_test = n_data - n_train - n_valid
            train_data, val_data, test_data = torch.utils.data.random_split(dataset, [n_train, n_valid, n_test])
            ...
            train_dataset = X_Image(train_data, number_samples=51000, dataset=args.dataset)
            test_dataset = X_Image(test_data, number_samples=9000, dataset=args.dataset)
            ...
            args.ClassGroup = True
            args.channels_list = [60, 120, 240, 400, 800, 1600]
            args.N_Classes = [20, 20, 20, 20, 200, 200]
            args.sf_min_testerror = 0.6

            train_data = TrainTinyImageNetDataset(id=id_dict)

            test_data = TestTinyImageNetDataset(id=id_dict)
            train_dataset = X_Tiny(train_data, number_samples=100000, dataset=args.dataset)
            test_dataset = X_Tiny(test_data, number_samples=10000, dataset=args.dataset)
